...
//	}, {
	...
	$.each(group, function(n, value) {		
		...
		{
			groupnum = groupnum + 1;
			$("#webgrid").append('<div id="group' + groupnum + '" class="row cells5"></div>');
		}
		
		$("#group" + groupnum).append('<div class="cell"> <h4>'+  value.n + '</h4> <ul id="' + value.g + '" class="numeric-list square-marker large-bullet"></ul> </div>');
		groupid = groupid + 1;
	});
	
	$.each(obj, function(n, value) {
		$("#" + value.g).append('<li>' + value.n + '</li>');
	});
});
